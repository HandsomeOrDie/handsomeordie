...
import { getOperationRisk, deleteOperationRisk, addOperationRisk, saveOperationRisk, setUnsaved, getSelectValue, setOperationRisk, setSelectValue } from "../../../actions/operationRisk";
...
class OperationRisk extends Component {
    ...
    handleSave = (row, column) => {
        ...
        const updateInfo = this.updateSelectable(column);
    ...
    updateSelectable = (column) => {
        let selectValue = this.props.selectValue;
        // const spotSymbol = ["any", "EURUSD","GBPUSD","USDTRY","USDCNY","USDCNH","EURJPY", "EURGBP", "USDJPY"];
        const spotSymbol = selectValue.symbolCopy;
        const criteriaCopy = selectValue.criteriaCopy;
        const forwardSymbol = ["any"];
        // const forwardSymbol = ["any", "EURUSD.1W","EURUSD.1M", "EURUSD.3M", "EURUSD.6M", "EURUSD.1Y", "USDCNY.1W","USDCNY.1M", "USDCNY.3M", "USDCNY.6M", "USDCNY.1Y"];
        const swapSymbol = ["any"];
        // const swapSymbol = ["any", "EURUSD.1W/1M","EURUSD.1M/3M", "EURUSD.1M/6M", "EURUSD.3M/6M", "USDCNY.1W/1M","USDCNY.1M/3M", "USDCNY.1M/6M", "USDCNY.3M/6M"];

        if (column) {
            if (column.dataIndex === "ccy") {
                if (column.value !== "any"){
                    selectValue.symbol = spotSymbol;
                    selectValue.criteria = criteriaCopy.filter((item)=>{
                        return item.text === "Exposure" || item.text === "Exposure Change";
                    });
                    console.log(selectValue.criteria);
                    this.props.setSelectValue(selectValue);
                    return {productType: "any", symbol: "any",};
                }else {
                    selectValue.criteria = criteriaCopy.filter((item)=>{
                        return item.text !== "Exposure" && item.text !== "Exposure Change";
                    });
            ...
            if (column.dataIndex === "productType") {
                if (column.value === "FORWARD"){
                    selectValue.symbol = forwardSymbol.map(item => ({text: item, value: item}));
                    this.props.setSelectValue(selectValue);
                    return {ccy: "any", symbol: "any"};
                }else if (column.value === "SWAP"){
                    selectValue.symbol = swapSymbol.map(item => ({text: item, value: item}));
                    this.props.setSelectValue(selectValue);
                    return {ccy: "any", symbol: "any"};
                }else if (column.value === "SPOT") {
                    selectValue.symbol = spotSymbol;
                    this.props.setSelectValue(selectValue);
                    return {ccy: "any"};
                }else if (column.value === "any"){
                    selectValue.symbol = spotSymbol;
                    this.props.setSelectValue(selectValue);
                }
            }
            if (column.dataIndex === "symbol") {
                if (column.value !== "any"){
                    return {productType: "SPOT", ccy: "any"};
                }
            }
        }
    }
    ...
    render() {
        ...
        return (
            <Table
                ...
                title={()=>(
                    <div>
                        <label style={{color: "rgb(63, 145, 142)", fontWeight: 700}}>Risk Engine Rule</label>
                        <span style={{float: "right"}}>
                            <a onClick={this.addOperationRisk} style={{marginRight: 10}}><Icon type="plus-circle" style={iconStyle}/></a>
                            <a onClick={this.deleteOperationRisk} style={{marginRight: 30}}><Icon type="minus-circle" style={{fontSize: 20}}/></a>
                ...
                bordered={true}
                bodyStyle={{height: tableHeight-190}}
                scroll={{x: 2500, y: tableHeight-190}}
