...
import { findHedge, findSelectOption, findHedgeSelectValue, findMarketRisk, updateMarketRisk, findCreditCpty } from "../../common/marketApi/index";

const getMarketRiskList = (params) => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: ActionTypes.MARKET_RISK_LOADING, payload: true });
            const result = await findMarketRisk(params);
            ...
            dispatch({ type: ActionTypes.MARKET_RISK_LIST, payload: data });
            dispatch({ type: ActionTypes.MARKET_RISK_UNSAVED, payload: false });
            dispatch({ type: ActionTypes.MARKET_RISK_LOADING, payload: false });
        } catch (e) {
            console.log(e);
        }
    };
...
const deleteMarketRisk= (key, list) => {
    return async dispatch => {
        try {
            list.splice(list.findIndex(item => item.key === key), 1);
            let arr = [];
            arr = arr.concat(list);
            dispatch({ type: ActionTypes.MARKET_RISK_UNSAVED, payload: true });
            dispatch({ type: ActionTypes.MARKET_RISK_LIST, payload: arr });
        } catch (e) {
            console.log(e);
        }
    };
...
const addMarketRisk= (list) => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: ActionTypes.MARKET_RISK_LIST, payload: [...list, newData] });
            dispatch({ type: ActionTypes.MARKET_RISK_UNSAVED, payload: true });
        } catch (e) {
            console.log(e);
        }
...
const saveMarketRisk = (list, cb) => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: ActionTypes.MARKET_RISK_LOADING, payload: true });
            const response = await updateMarketRisk(list);
            cb(response);
            dispatch({ type: ActionTypes.MARKET_RISK_UNSAVED, payload: false });
            dispatch({ type: ActionTypes.MARKET_RISK_LOADING, payload: false });
        } catch (e) {
...
const getSelectValue = () => {
    let data = {
        ...
        criteria: [
            ...
            {text: "Exposure Change", value: "Exposure Change"},
        ],
        direction: [
            {text: ">", value: ">"},
            {text: ">=", value: ">="},
    ...
    return async dispatch => {
        try {
            const anyField = ["group", "riskUser", "book", "productType", "counterParty", "ccy", "symbol", "window", "strategy","dimension",];
            const baseField = ["pricingCcy"];
            let response = await findSelectOption();
            ...
            Object.keys(selectValue).map(item => {
                ...
                // data[item].splice(0, 0, {text: "-", value: "-"});
            });
            ...
            dispatch({ type: ActionTypes.MARKET_RISK_SELECT_VALUE, payload: data });
        } catch (e) {
            console.log(e);
        }
    };
...
const setMarketRisk = (list) => {
    return async dispatch => {
        try {
            dispatch({ type: ActionTypes.MARKET_RISK_LIST, payload: list});
        } catch (e) {
            console.log(e);
        }
    };
...
const setSelectValue = (selectValue) => {
    return async dispatch => {
        try {
            dispatch({ type: ActionTypes.MARKET_SELECT_VALUE, payload: {...selectValue} });
        } catch (e) {
            console.log(e);
        }
    };
...
const showMarket = (params) => {
    return async dispatch => {
        try {
            dispatch({ type: ActionTypes.CHANGE_SHOW_MARKET, payload: params});
        } catch (e) {
            console.log(e);
        }
    };
...
const showOperation = (params) => {
    return async dispatch => {
        try {
            dispatch({ type: ActionTypes.CHANGE_SHOW_OPERATION, payload: params});
        } catch (e) {
            console.log(e);
        }
    };
